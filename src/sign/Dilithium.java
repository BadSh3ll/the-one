...
public class Dilithium {
    ...
    public final static int gamma1 = 1 << 17;
    public final static int gamma2 = (Q - 1) / 88;
    public final static int tau = 39;
    public final static int d = 13;
    public final static int chalentropy = 192;
    public final static int eta = 2;
    public final static int beta = 78;
    public final static int omega = 80;
    ...
    public static KeyPair keygen(byte[] seed) {
        ...
        PolyVec s1 = PolyVec.randomVec(rhoprime, eta, l, 0);
        PolyVec s2 = PolyVec.randomVec(rhoprime, eta, k, l);

        // Generate A
        PolyVec[] A = expandA(rho, k, l);
        ...
        byte[] prvbytes = PackingUtils.packPrvKey(eta, rho, tr, K, res[0], s1, s2);

        PolyVec s2hat = s2.ntt();
        PolyVec t0hat = res[0].ntt();

        DilithiumPrivateKey prv = new DilithiumPrivateKey(rho, K, tr, s1, s2, res[0], prvbytes, A, s1hat, s2hat, t0hat);
        DilithiumPublicKey pub = new DilithiumPublicKey(rho, res[1], pubbytes, A);
    ...
    public static byte[] sign(DilithiumPrivateKey prv, byte[] M) {
		...
		final int CRYPTO_BYTES = Utils.getSigLength();

		byte[] sig = new byte[CRYPTO_BYTES];
		
		PolyVec[] A;
		if(prv instanceof DilithiumPrivateKey) {
			A = ((DilithiumPrivateKey)prv).getA();
		}
		else {
			A = expandA(prv.getRho(), k, l);	
		}
		...
		for (;;) {
			...
			PackingUtils.packw1(gamma2, res[1], sig);

			SHAKEDigest s = new SHAKEDigest(256);
			s.update(mu, 0, mu.length);
			s.update(sig, 0, res[1].length() * PackingUtils.getPolyW1PackedBytes(gamma2));
			s.doOutput(sig, 0, SEEDBYTES);

			Poly cp = generateChallenge(tau, sig);
			cp = cp.ntt();
			...
			Hints hints = makeHints(gamma2, w0, res[1]);
			if (hints.cnt > omega) {
				continue;
			}

			PackingUtils.packSig(gamma1, omega, sig, sig, z, hints.v);
			return sig;
		}
	}

    public static boolean verify(DilithiumPublicKey pk, byte[] sig, byte[] M) {
        int CRYPTO_BYTES = Utils.getSigLength();
        ...
        for (int i = 0; i < l; i++) {
            z.poly[i] = PackingUtils.zunpack(gamma1, sig, off);
            off += PackingUtils.getPolyZPackedBytes(gamma1);
        }

        PolyVec h = new PolyVec(k);
        int k = 0;
        for (int i = 0; i < h.length(); i++) {
            h.poly[i] = new Poly(N);

            if ((sig[off + omega + i] & 0xFF) < k || (sig[off + omega + i] & 0xFF) > omega)
                throw new RuntimeException("Bad signature");

            for (int j = k; j < (sig[off + omega + i] & 0xFF); j++) {
        ...
        Poly cp = generateChallenge(tau, c);

        PolyVec[] A;
        if (pk instanceof DilithiumPublicKey) {
            A = ((DilithiumPublicKey) pk).getA();
        } else {
            A = expandA(pk.getRho(), k, l);
        }
        z = z.ntt();
        PolyVec w = z.mulMatrixPointwiseMontgomery(A);
        ...
        w = useHint(gamma2, w, h);

        byte[] buf = new byte[PackingUtils.getPolyW1PackedBytes(gamma2) * w.length()];
        PackingUtils.packw1(gamma2, w, buf);
